...
function Pipeline( streams, options ) {
  ...
  this._errorHandler = function( error, ...argv ) {
    error.stream = this
    self.emit( 'error', error, ...argv )
  }
...
Pipeline.prototype = {
   ...
   */
  append( ...streams ) {
    this.splice( this._streams.length, 0, ...streams )
    return this
  },
  ...
  splice( index, remove, ...streams ) {
    ...
    var removed = this._streams.splice( index, remove, ...streams )
